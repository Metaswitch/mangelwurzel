...
class MangelwurzelTest : public SipCommonTest
{
public:
  ...
  MangelwurzelTest() : SipCommonTest()
  {
  }
...
TEST_F(MangelwurzelTest, InitialReq)
{
  ...
  EXPECT_CALL(*_helper, get_pool(req)).WillOnce(Return(_pool));
...
TEST_F(MangelwurzelTest, Response)
{
  ...
  EXPECT_CALL(*_helper, get_pool(rsp)).WillOnce(Return(_pool));
